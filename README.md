...
|-----------|------|
| `mc` | Minecraft サーバー本体 |
| `mc-tailscale` | Tailscale サイドカー（リモートアクセス） |
| `mc-backups` | 自動バックアップ |
| `mc-scheduler` | スケジュール起動停止・日次再起動 |
| `mc-autoheal` | クラッシュ自動復旧 |
| `mc-minedmap` | Web マップ タイル生成 |
| `mc-map-viewer` | Web マップ 表示（nginx） |
...
サーバー起動と同時に、ブラウザで閲覧できる2Dマップが自動生成されます。

- **アクセス:** `http://localhost:8123`（ポートは `.env` の `MAP_PORT` で変更可能）
- **更新:** ワールドデータの変更を自動検知し、増分レンダリング（変更チャンクのみ再生成）
- **CPU負荷:** 増分更新は数秒で完了。ゲームプレイへの影響はほぼなし
- **mod不要:** サーバー本体には一切触れず、保存されたregionファイルを外部から読み取る

### 初回レンダリング

初回起動時は全チャンクのレンダリングが必要なため、ワールドサイズに応じて数分〜十数分かかります。レンダリング中もサーバーは通常通りプレイ可能です。

### マップにピンを立てる

ゲーム内で看板を設置し、1行目に `[Map]` と書くとマップ上にマーカーが表示されます。

```
[Map]拠点
```

`[Map]` の後のテキストがマーカーの名前になります。
